...
nlcd_agg <- st_warp(nlcd,
    cellsize = 10000, method = 'mode', use_gdal = TRUE)
nlcd_agg@legend <- nlcd@legend
plot(nlcd_agg)
```

Here, `cellsize = 10000` means aggregate to 10-km resolution (the raster's projection
is in units of meters) and `method = 'mode'` indicates that the aggregated value is the 
mode of the original pixels
(averaging would not work since land cover is a categorical variable). 
`use_gdal = TRUE` ensures that we use the GDAL system library behind the scenes to
do the aggregation.
{:.notes}
