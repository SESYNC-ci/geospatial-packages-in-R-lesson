...
for reading and working with raster data. Recently, the developers of 
...
family of packages, and has significant performance advantages. Older versions of this lesson
...
The index `[1, 1]` corresponds to the bottom left pixel which matches our
intuition for a two-dimensional image with `x` and `y` coordinates. The "origin"
...
Here, `cellsize = dim(nlcd)/10` divides the pixel dimensions of the original
raster by 10 and sets that as the dimensions of the downsampled raster. Because
the original `nlcd` raster has 150-meter pixels, this results in 1500-meter resolution.
*note: as of May 2021 the documentation is incorrect. It says `cellsize` is the 
size of each pixel but it is actually the dimensions of the output.*
The argument `method = 'mode'` indicates that the aggregated value is the 
mode of the original smaller pixels making up each individual larger output pixel
